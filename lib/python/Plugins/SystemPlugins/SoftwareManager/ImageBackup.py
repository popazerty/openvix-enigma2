...
class ImageBackup(Screen):
	...
	def doFullBackup(self, DIRECTORY):
		...
		if self.ROOTFSTYPE == "ubi":
			...
			self.message += _("will take between 3 and 12 minutes for this system\n")
		elif "tar.bz2" in self.ROOTFSTYPE.split():
			self.message += _("because of the used filesystem the backup\n")
			self.message += _("will take between 1 and 4 minutes for this system\n")
		else:
	...
	def doFullBackupCB(self):
		...
		if file_found:
			...
			cmdlist.append('echo "     wrong backup destination "')
			cmdlist.append('echo "     no space left on backup device"')
			cmdlist.append('echo "     no writing permission on backup device"')
			cmdlist.append('echo " "')
		...
		if self.DIRECTORY == "/hdd":
			...
			if self.TARGET == 'XX':
				...
				cmdlist.append('echo "_________________________________________________\n"')
				cmdlist.append('echo " "')
				cmdlist.append('echo "A valid USB drive was detected in one "')
				cmdlist.append('echo "of the USB ports, therefore an extra copy of the "')
				cmdlist.append('echo "backup image is now being copied to that USB "')
				cmdlist.append('echo "flash drive. "')
				cmdlist.append('echo "This will only take 1 or 2 minutes"')
				cmdlist.append('echo " "')
